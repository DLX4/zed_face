...
//dlx------------------------will modify here to save a bmp picture as the background
FILE * fp = fopen("/background.jpg","wb+");
if(!fp){printf("open temp.jpg error\n");}

FILE * fp1 = fopen("/background.bmp","wb+");
if(!fp1){printf("open temp.bmp error\n");}

fwrite(pglobal->in[input_number].buf,frame_size,1,fp);
fseek(fp,0,SEEK_SET);
read_jpeg_file(fp,fp1);
fseek(fp,0,SEEK_SET);
...
void free_frame_buffer()
{
	...
	for(i =0;i<10;i++)
	{
		//if((frame_buffer.timestamps[i].tv_sec != 0)
			//&& (frame_buffer.timestamps[i].tv_usec != 0))
			if(frame_buffer.frame_array[i] != 0)
			{
				free(frame_buffer.frame_array[i]);
			}
	}
}
void buffer_init()
{
	...
	for(i =0;i<10;i++)
	{
		frame_buffer.timestamps[i].tv_sec = 0;
		frame_buffer.timestamps[i].tv_usec = 0;
		frame_buffer.frame_array[i] = 0;
	}
	
	
}
int  find_next_free_frame()
{
	int  i;
	for(i =0;i<10;i++)
	{
...
int get_next_frame()
{
	int  i ;
	int  min = 0;
	//if(frame_buffer.full_flag == 0)
	//{
		//return -1;
	//}
	if(find_next_free_frame() >= 0)
	{
		return -1;
	}
	for(i = 0;i < 10;i++)
	{
		/*
		if( (frame_buffer.timestamps[i].tv_sec <= frame_buffer.timestamps[min].tv_sec) 
			&& (frame_buffer.timestamps[i].tv_usec < frame_buffer.timestamps[min].tv_usec)
			&& (frame_buffer.timestamps[i].tv_sec != 0)
			&& (frame_buffer.timestamps[i].tv_usec != 0) 
			)
		{
		...
		*/
		if((frame_buffer.timestamps[i].tv_sec == frame_buffer.timestamps[min].tv_sec)
			...
			{
				min = i;
			}else if((frame_buffer.timestamps[i].tv_sec < frame_buffer.timestamps[min].tv_sec))
			{
				min = i;
			}
	}
...
void send_stream(int fd, int input_number)
{
	...
	//add dlx------------
	printf("inint000000000000000000000000000000 \n");
	//buffer_init();
    ...
    while(!pglobal->stop) {
        ...
        int next_frame = find_next_free_frame();//must between 0 -10
        printf("buffer====================================next ---%d\n",next_frame);
        max_frame_size = frame_buffer.frame_sizes[next_frame];
//printf("buffer====================================next size--%d  ,,frame size%d -\n",max_frame_size,frame_size);
        /* check if framebuffer is large enough, increase it if necessary */
        if(frame_size > max_frame_size) {
			    DBG("increasing buffer size to %d\n", frame_size);
		//printf("buffer====================================0.1\n");
			    max_frame_size = frame_size + TEN_K;
			    if((tmp = realloc(frame_buffer.frame_array[next_frame], max_frame_size)) == NULL) {
				free(frame_buffer.frame_array[next_frame]);
				pthread_mutex_unlock(&pglobal->in[input_number].db);
				send_error(fd, 500, "not enough memory");
				return;
			    			}
			    frame_buffer.frame_array[next_frame] = tmp;
			    frame_buffer.frame_sizes[next_frame] = max_frame_size;
			    //printf("buffer====================================1\n");
            // dlx ----- alloc  more frame buffer
        ...
        /* copy v4l2_buffer timeval to user space */
        //timestamp = pglobal->in[input_number].timestamp;
        frame_buffer.timestamps[next_frame] =pglobal->in[input_number].timestamp;
        		//frame_buffer.timestamps[next_frame].tv_usec = timestamp.tv_usec;
//printf("buffer====================================0.3    %x\n",frame_buffer.frame_array[next_frame]);
//printf("timestamp put new %d.%06d \n",frame_buffer.timestamps[next_frame].tv_sec,frame_buffer.timestamps[next_frame].tv_usec);
        memcpy(frame_buffer.frame_array[next_frame], pglobal->in[input_number].buf, frame_size);
...
//printf("buffer====================================0.4\n");
         ...
         * with firefox
         */
 //dlx -------get frame from the buffer
 //printf("buffer====================2......find_next_free_frame()= %d\n",find_next_free_frame());
		 if(find_next_free_frame() == -1)
		 {
		 //printf("buffer====================================3s\n");
				int dlx = get_next_frame();//must
				 frame_size = frame_buffer.frame_sizes[dlx]; 
				 timestamp.tv_sec = frame_buffer.timestamps[dlx].tv_sec;
				 timestamp.tv_usec = frame_buffer.timestamps[dlx].tv_usec;
				 frame_buffer.timestamps[dlx].tv_sec = 0;
				frame_buffer.timestamps[dlx].tv_usec = 0;
				printf("send----------------------------------------%d   timestamp %d.%06d \n",dlx,(int)timestamp.tv_sec,(int)timestamp.tv_usec);
				 //dlx put the unlock here 
				 pthread_mutex_unlock(&pglobal->in[input_number].db);
				 //printf("buffer----------------------------------------%d   timestamp %d.%06d \n",dlx,(int)timestamp.tv_sec,(int)timestamp.tv_usec);
					...
					DBG("sending frame\n");
					if(write(fd, frame_buffer.frame_array[dlx], frame_size) < 0) break;
