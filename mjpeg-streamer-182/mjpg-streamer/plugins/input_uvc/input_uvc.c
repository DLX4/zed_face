...
/* add by dlx for face detect */
static int face_detect_interval = 5 ;
static int face_detect_count =0;
pthread_mutex_t mutex_face_detect;
static volatile int face_detect_finished = 0;
static volatile int face_detect_ready = 0;
void * pHandle ;
int (*pmyfun)();
void *face_detection_thread(void);
/* end add by dlx for face detect */
...
//dlx init the face detect lock
pthread_mutex_init(&mutex_face_detect,0);
//int (*pmyfun)();
pHandle = dlopen("/libface_detection.so",RTLD_LAZY);
pmyfun = (int (*)())dlsym(pHandle,"_Z9face_mainv");
...
int face_detect_threadID = 0;
	pthread_create(&face_detect_threadID,NULL,face_detection_thread,NULL);
printf("pid ---------------------%d\n",face_detect_threadID);
	pthread_detach(face_detect_threadID);
    return 0;
}
...
void *face_detection_thread(void)
{
	while(1)
	{
		pthread_mutex_lock(&mutex_face_detect);
		if(face_detect_ready == 1)
		{
			printf("detect******************************************\n");
			//run the face detect in another thread
			//pthread_mutex_unlock(&mutex_face_detect);
			pmyfun();
			//pthread_mutex_lock(&mutex_face_detect);
			//pthread_mutex_lock(&mutex_face_detect);
			face_detect_finished = 1;
			//pthread_mutex_unlock(&mutex_face_detect);
			face_detect_ready = 0;
			pthread_mutex_unlock(&mutex_face_detect);
		}else{
...
//dlx add
	struct timeval face_detect_timestamp={0};
    ...
    while(!pglobal->stop) {
        ...
        if(pcontext->videoIn->formatIn == V4L2_PIX_FMT_YUYV) {
            ...
            pglobal->in[pcontext->id].size = compress_yuyv_to_jpeg(pcontext->videoIn, pglobal->in[pcontext->id].buf, pcontext->videoIn->framesizeIn, gquality);
				...
				//if((face_detect_count++)%7 == 6) {

						if(pthread_mutex_trylock(&mutex_face_detect) == 0)//get the lock
						{
							printf("get the lock------------1\n");
							if((face_detect_ready == 0)&&(face_detect_finished == 0))
							{
								/*   before face detect */
								FILE * fp = fopen("/temp.jpg","wb+");
								if(!fp){printf("open temp.jpg error\n");}

								FILE * fp1 = fopen("/temp.bmp","wb+");
								if(!fp1){printf("open temp.bmp error\n");}

								fwrite(pcontext->videoIn->tmpbuffer,pcontext->videoIn->buf.bytesused,1,fp);
								fseek(fp,0,SEEK_SET);
								...
								fclose(fp);
								face_detect_ready = 1;
								//dlx ----save the bmp timestamp and go to the next loop
								face_detect_timestamp = pcontext->videoIn->buf.timestamp;
								pthread_mutex_unlock(&mutex_face_detect);
								printf("^^^^^^^^^^^^^^^^^^^^^^^^^fist save the tmp  and continue\n");
						...
						if(pthread_mutex_trylock(&mutex_face_detect) == 0)//get the lock
						{
							printf("get the lock------------2\n");
							if(face_detect_finished == 1)
							{

								read_bmp_header("/tmp_detection.bmp");
								//pthread_mutex_unlock(&mutex_face_detect);
								pglobal->in[pcontext->id].size = compress_bmp_to_jpeg(pglobal->in[pcontext->id].buf,80);

								//pthread_mutex_lock(&mutex_face_detect);
								face_detect_finished = 0;

								//dlx ---add the old timestamp and save the current bmp
								pglobal->in[pcontext->id].timestamp = face_detect_timestamp;
								printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~dlx add the timestamp\n");
								printf("~~~~~~   timestamp %d.%06d \n",(int)face_detect_timestamp.tv_sec,(int)face_detect_timestamp.tv_usec);
								...
								//dlx ----save the bmp timestamp and go to the next loop
								face_detect_timestamp = pcontext->videoIn->buf.timestamp;
								printf("^^^^^^^^^^^^^^^^^^^^^^^^^^^^dlx save the tmp  and continue\n");
				...
				pthread_mutex_unlock(&pglobal->in[pcontext->id].db);
						...
						printf("pcontext->videoIn->fps------%d\n",pcontext->videoIn->fps);
